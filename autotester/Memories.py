...
class PostgresMemory(DillSave):
    ...
    def __init__(self, pg_path, sync_every=10):
        ...
        self.last_id =None
        db_id=self.db["run"].find_one(order_by=["-id"],_limit=1)
        if db_id is not None:
          self.last_id=db_id.get("id")
    ...
    def sync(self,chunk_size):
        while len(self.test_log_local)>0:
            ...
            if hasattr(self,"CURRENT_SAMPLE"):
              log_dict["sample"]={"sample_id":self.CURRENT_SAMPLE,"log_local_id":temp_log.local_id,"run_id":temp_log.run_id}
